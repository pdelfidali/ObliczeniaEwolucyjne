...
def individual(icls):
    genome = list()
    for x in range(0, 2 * bitsLength):
        genome.append(random.randint(0, 1))
    return icls(genome)


def decode_bits(binary_representation):
    return minValue + int(''.join(map(str, binary_representation)), 2) * (
            maxValue - minValue) / (2 ** bitsLength - 1)


def decode_ind(_individual):
    return decode_bits(_individual[:bitsLength]), decode_bits(_individual[bitsLength:])
...
crossing_dict = {
    'one': tools.cxOnePoint,
    'uniform': tools.cxUniform,
    'two': tools.cxTwoPoint,
}

mutation_dict = {
    'shuffleIndexes': tools.mutShuffleIndexes,
    'flipBit': tools.mutFlipBit,
}
